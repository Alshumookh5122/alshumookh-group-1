...
from app.alchemy_service import verify_alchemy_signature
...
async def alchemy_webhook(
    ...
    await log_event(db, 'ALCHEMY_WEBHOOK', payload)
    return WebhookAck()
...
async def _find_order_for_transak_event(db: AsyncSession, decoded: dict) -> PaymentOrder | None:
    ...
    for external_id in [str(x) for x in candidate_ids if x]:
        result = await db.execute(
            select(PaymentOrder)
            .where(PaymentOrder.provider == Provider.TRANSAK, PaymentOrder.external_id == external_id)
    ...
    wallet = event_data.get('walletAddress') or event_data.get('wallet_address') or decoded.get('walletAddress')
    crypto_amount = event_data.get('cryptoAmount') or event_data.get('crypto_amount') or decoded.get('cryptoAmount')
    network = _normalize_network(event_data.get('network') or decoded.get('network'))
    ...
    if wallet:
        result = await db.execute(
            ...
            .where(
                ...
                PaymentOrder.status.in_([OrderStatus.CREATED, OrderStatus.PENDING, OrderStatus.PROCESSING]),
            )
            .order_by(PaymentOrder.created_at.desc())
        )
...
async def transak_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        decoded = jwt.decode(token, access_token, algorithms=['HS256', 'HS512'], options={'verify_aud': False})
    except jwt.PyJWTError as exc:
        raise HTTPException(status_code=401, detail=f'Invalid Transak webhook token: {exc}') from exc
    ...
    if not order:
        await log_event(db, 'TRANSAK_ORDER_NOT_FOUND', {'decoded': decoded}, None)
        return WebhookAck()
    ...
    if event_type in status_map:
        ...
        if order.status in {OrderStatus.FAILED, OrderStatus.REFUNDED, OrderStatus.EXPIRED}:
            event_data = _event_data(decoded)
            order.failure_reason = event_data.get('reason') or decoded.get('message')
        await db.commit()
        await log_event(db, 'ORDER_STATUS_SYNCED', {'status': order.status.value}, order.id)
        return WebhookAck()

    await log_event(db, 'TRANSAK_WEBHOOK_UNHANDLED', {'event_type': event_type, 'decoded': decoded}, order.id)
    await db.commit()
    return WebhookAck()
