...
.curr-card{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:20px;margin-bottom:16px;transition:border-color .2s;}
.curr-card.deployed{border-color:rgba(124,58,237,.4);}
.curr-badge{display:inline-flex;align-items:center;padding:3px 16px;border-radius:20px;font-size:14px;font-weight:800;background:rgba(251,191,36,.12);color:#fbbf24;border:1px solid rgba(251,191,36,.25);}
.investor-box{margin-top:14px;padding:14px;background:rgba(124,58,237,.05);border:1px solid rgba(124,58,237,.2);border-radius:10px;}
.investor-box h4{font-size:12px;font-weight:700;color:#c4b5fd;margin:0 0 12px;}
.prow{display:grid;grid-template-columns:1fr 150px 90px;gap:6px;align-items:center;margin-bottom:6px;}
.manage-area{border-top:1px solid var(--border);margin-top:14px;padding-top:14px;display:none;}
.dist-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;}
.dist-grid.three{grid-template-columns:1fr 1fr 1fr;}
.dist-stat{background:rgba(255,255,255,.04);border-radius:8px;padding:10px 12px;}
.dist-stat label{font-size:10px;color:var(--muted);display:block;margin-bottom:3px;text-transform:uppercase;}
.dist-stat span{font-size:13px;font-weight:700;color:#f0f0f0;}
.payee-item{display:flex;gap:10px;padding:6px 10px;background:rgba(255,255,255,.03);border-radius:6px;margin-bottom:4px;font-size:11px;}
.payee-item code{flex:1;color:var(--muted);}
.payee-item span{font-weight:700;color:#a78bfa;}
...
.pay-table td{padding:7px 8px;border-bottom:1px solid rgba(255,255,255,.04);vertical-align:middle;}
</style>

<!-- ethers.js multi-CDN sequential loader -->
<script>
(function(){
  var cdns=[
    ...
    "https://unpkg.com/ethers@6.7.1/dist/ethers.umd.min.js"
  ];
  var idx=0;
  function nxt(){
    if(idx>=cdns.length){console.error("ethers.js: all CDNs failed");return;}
...
<div class="wallet-bar">
  <span style="font-size:20px;">&#x1F98A;</span>
  <code id="walletAddr" style="flex:1;color:#c4b5fd;font-size:12px;">Not connected</code>
  ...
  <button class="dist-btn primary" id="connectBtn" onclick="connectWallet()">Connect MetaMask</button>
  <button class="dist-btn ghost sm" onclick="diagWallet()">&#x1F50D; Diag</button>
</div>
<div id="diagBox" style="display:none;margin-bottom:12px;padding:12px 16px;background:rgba(124,58,237,.08);border:1px solid rgba(124,58,237,.3);border-radius:10px;font-size:12px;line-height:1.9;"></div>

<!-- Currency Cards (rendered by JS) -->
<div id="registryArea"><p style="color:var(--muted);font-size:12px;">Loading...</p></div>

<!-- Deposit / Claim Panel -->
<div class="dist-card" id="dcPanel" style="display:none;border-color:rgba(5,150,105,.3);">
  <h3 id="dcTitle" style="color:#34d399;">Deposit / Claim</h3>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
    <div>
      <label style="font-size:11px;color:var(--muted);">Deposit Amount</label>
      <input class="dist-input" id="depositAmount" placeholder="e.g. 0.5 or 1000" style="margin:6px 0 8px;" />
      <button class="dist-btn success sm" onclick="doDeposit()">Deposit</button>
      <p style="font-size:11px;color:var(--muted);margin-top:6px;">ETH: payable value. ERC-20: approve then deposit.</p>
    </div>
    <div>
      <label style="font-size:11px;color:var(--muted);">Investor Address</label>
      <input class="dist-input" id="claimInvestor" placeholder="0x... investor wallet" style="margin:6px 0 8px;" />
  ...
  <div style="display:flex;gap:8px;margin-top:14px;border-top:1px solid var(--border);padding-top:12px;flex-wrap:wrap;">
    ...
    <button class="dist-btn ghost sm" onclick="closeDCPanel()">Close</button>
  </div>
</div>
...
<div class="dist-card">
  ...
  <p style="font-size:12px;color:var(--muted);margin:0 0 12px;">Link on-chain tx hashes to payment records. Stored locally.</p>
...
var ERC20_ABI=["function approve(address spender, uint256 amount) external returns (bool)","function decimals() view returns (uint8)"];
var DEPLOY_ABI=["constructor(address initialOwner)"];
var ZERO="0x0000000000000000000000000000000000000000";

var DEPLOY_BYTECODE = "";
var CURRENCY_DEFS=[
...
var provider,signer,walletAddress,currentDCSym=null;
function getRegistry(){try{return JSON.parse(localStorage.getItem("dist_registry")||"{}");}catch(e){return {};}}
function saveRegistry(r){localStorage.setItem("dist_registry",JSON.stringify(r));}
function bpsOptions(sel){
  var h="";
  for(var i=0;i<BPS_PRESETS.length;i++){
    var p=BPS_PRESETS[i];
    h+="<option value='"+p.v+"'"+(String(p.v)===String(sel)?" selected":"")+">" +p.label+"</option>";
  }
  return h;
}
function renderRegistry(){
  var reg=getRegistry(),h="";
  for(var i=0;i<CURRENCY_DEFS.length;i++){
    var c=CURRENCY_DEFS[i],entry=reg[c.sym]||{},ca=entry.contractAddr||"",wc=entry.walletCount||2;
    h+="<div class='curr-card"+(ca?" deployed":"")+"' id='card-"+c.sym+"'>";
    h+="<div style='display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap;'>";
    h+="<span class='curr-badge'>"+c.sym+"</span>";
    h+="<div style='flex:1;font-size:11px;color:var(--muted);'>";
    if(ca){h+="Contract: <code style='color:#a78bfa;font-size:10px;'>"+ca.slice(0,10)+"..."+ca.slice(-6)+"</code>";}
    else{h+="<span style='color:#f87171;'>No contract deployed</span>";}
    h+="</div>";
    if(ca){
      h+="<button class='dist-btn ghost sm' data-sym='"+c.sym+"' onclick='openDC(this.dataset.sym)'>Deposit/Claim</button>";
      h+="<button class='dist-btn ghost sm' data-sym='"+c.sym+"' onclick='toggleStats(this.dataset.sym)'>Stats</button>";
    }
    h+="</div>";
    h+="<div style='display:flex;gap:8px;margin-bottom:14px;'>";
    h+="<input class='dist-input' id='caInput-"+c.sym+"' placeholder='Paste existing contract (0x...) or deploy below' value='"+ca+"' style='margin:0;flex:1;font-size:11px;font-family:monospace;' />";
    h+="<button class='dist-btn ghost sm' data-sym='"+c.sym+"' onclick='saveCA(this.dataset.sym)'>Save</button></div>";
    h+="<div class='investor-box'><h4>&#x1F465; Investor Wallets &amp; Profit Share</h4>";
    h+="<div style='display:flex;align-items:center;gap:12px;margin-bottom:12px;flex-wrap:wrap;'>";
    h+="<label style='font-size:11px;color:var(--muted);'>Investors:</label>";
    h+="<select class='dist-input' id='wcount-"+c.sym+"' data-sym='"+c.sym+"' onchange='rebuildWallets(this.dataset.sym)' style='width:80px;margin:0;'>";
    for(var n=2;n<=10;n++){h+="<option value='"+n+"'"+(wc===n?" selected":"")+">" +n+"</option>";}
    h+="</select><span id='bpsSum-"+c.sym+"' style='font-size:12px;font-weight:700;color:#fbbf24;'>Total: 0 / 10000 BPS</span></div>";
    h+="<div id='wrows-"+c.sym+"' style='margin-bottom:10px;'></div>";
    h+="<p style='font-size:11px;color:var(--muted);margin:0 0 12px;'>10000 BPS = 100% &nbsp;|&nbsp; 5000 = 50% &nbsp;|&nbsp; 3000 = 30%</p>";
    h+="<div style='display:flex;gap:8px;flex-wrap:wrap;'>";
    if(!ca){h+="<button class='dist-btn primary' data-sym='"+c.sym+"' id='deployBtn-"+c.sym+"' onclick='deployAndSetPayees(this.dataset.sym)'>&#x1F680; Deploy &amp; Set Payees</button>";}
    else{
      h+="<button class='dist-btn primary sm' data-sym='"+c.sym+"' onclick='doSetPayees(this.dataset.sym)'>&#x1F4BE; Save Payees to Chain</button>";
      h+="<button class='dist-btn danger sm' data-sym='"+c.sym+"' id='deployBtn-"+c.sym+"' onclick='deployAndSetPayees(this.dataset.sym)'>&#x1F504; Redeploy</button>";
    }
    h+="</div><div id='deployStatus-"+c.sym+"' style='margin-top:8px;font-size:12px;color:var(--muted);'></div></div>";
    h+="<div class='manage-area' id='statsArea-"+c.sym+"'>";
    h+="<div style='display:flex;align-items:center;gap:10px;margin-bottom:10px;'>";
    h+="<span style='font-size:12px;font-weight:700;color:#c4b5fd;'>&#x1F4CA; Stats</span>";
    h+="<button class='dist-btn ghost sm' data-sym='"+c.sym+"' onclick='loadStatsFor(this.dataset.sym)'>Refresh</button>";
    h+="<button class='dist-btn ghost sm' data-sym='"+c.sym+"' onclick='closeStats(this.dataset.sym)'>&#x2715;</button></div>";
    h+="<div id='statsContent-"+c.sym+"'><p style='color:var(--muted);font-size:12px;'>Click Refresh to load.</p></div></div>";
    h+="</div>";
  }
  document.getElementById("registryArea").innerHTML=h;
  for(var j=0;j<CURRENCY_DEFS.length;j++){buildWalletRows(CURRENCY_DEFS[j].sym);}
}
function buildWalletRows(sym){
  var reg=getRegistry(),entry=reg[sym]||{},count=parseInt(document.getElementById("wcount-"+sym).value)||2;
  var saved=entry.walletData||[],container=document.getElementById("wrows-"+sym),h="";
  for(var i=0;i<count;i++){
    var addr=(saved[i]&&saved[i].addr)||"",bps=(saved[i]&&saved[i].bps)||(i===0?7000:(i===1?3000:0));
    h+="<div class='prow' id='prow-"+sym+"-"+i+"'>";
    h+="<input class='dist-input pb-addr' data-sym='"+sym+"' placeholder='Wallet #"+(i+1)+" (0x...)' value='"+addr+"' style='margin:0;' oninput='updateBpsSum(this.dataset.sym)' />";
    h+="<select class='dist-input pb-bps' data-sym='"+sym+"' style='margin:0;' onchange='onBpsSelChange(this)'>"+bpsOptions(bps)+"</select>";
    h+="<input class='dist-input pb-custom' type='number' min='1' max='10000' placeholder='BPS' data-sym='"+sym+"' style='margin:0;display:"+(String(bps)==="custom"?"block":"none")+";' oninput='updateBpsSum(this.dataset.sym)' />";
    h+="</div>";
  }
  container.innerHTML=h;
  reg[sym]=reg[sym]||{};reg[sym].walletCount=count;saveRegistry(reg);updateBpsSum(sym);
}
function rebuildWallets(sym){buildWalletRows(sym);}
function onBpsSelChange(sel){
  sel.closest(".prow").querySelector(".pb-custom").style.display=sel.value==="custom"?"block":"none";
  updateBpsSum(sel.dataset.sym);
}
function updateBpsSum(sym){
  var rows=document.querySelectorAll("#wrows-"+sym+" .prow"),total=0;
  rows.forEach(function(row){
    var sel=row.querySelector(".pb-bps");
    total+=sel.value==="custom"?(parseInt(row.querySelector(".pb-custom").value)||0):(parseInt(sel.value)||0);
  });
  var lbl=document.getElementById("bpsSum-"+sym);
  if(lbl){lbl.textContent="Total: "+total+" / 10000 BPS";lbl.style.color=total===10000?"#34d399":(total>10000?"#f87171":"#fbbf24");}
}
function getWalletInputs(sym){
  var rows=document.querySelectorAll("#wrows-"+sym+" .prow"),addrs=[],shares=[],ok=true;
  rows.forEach(function(row,idx){
    var addr=row.querySelector(".pb-addr").value.trim();
    var sel=row.querySelector(".pb-bps"),bps=sel.value==="custom"?(parseInt(row.querySelector(".pb-custom").value)||0):(parseInt(sel.value)||0);
    if(!addr||!/^0x[0-9a-fA-F]{40}$/.test(addr)){alert("Invalid address for Investor #"+(idx+1));ok=false;}
    addrs.push(addr);shares.push(bps);
  });
  if(!ok) return null;
  var sum=shares.reduce(function(a,b){return a+b;},0);
  if(sum!==10000){alert("BPS total is "+sum+" — must equal 10000");return null;}
  return {addrs:addrs,shares:shares};
}
function getCurrDef(sym){return CURRENCY_DEFS.find(function(c){return c.sym===sym;});}
function getContractFor(sym){
  if(typeof ethers==="undefined"){alert("ethers.js not loaded yet — wait and retry.");return null;}
  if(!signer){alert("Connect MetaMask first");return null;}
  var reg=getRegistry(),ca=(reg[sym]&&reg[sym].contractAddr)||"";
  if(!ca){alert("No contract for "+sym+" — deploy first.");return null;}
  return new ethers.Contract(ca,ABI,signer);
}
function saveCA(sym){
  var val=(document.getElementById("caInput-"+sym)||{}).value||"";
  val=val.trim();
  if(val&&!/^0x[0-9a-fA-F]{40}$/.test(val)){alert("Invalid Ethereum address");return;}
  var reg=getRegistry();reg[sym]=reg[sym]||{};reg[sym].contractAddr=val;saveRegistry(reg);
  log("Contract saved for "+sym+": "+(val||"(cleared)"));renderRegistry();showToast("Contract saved!","ok");
}
function toggleStats(sym){
  var el=document.getElementById("statsArea-"+sym);
  if(!el) return;
  if(el.style.display==="none"||!el.style.display){el.style.display="block";loadStatsFor(sym);}
  else el.style.display="none";
}
function closeStats(sym){var el=document.getElementById("statsArea-"+sym);if(el)el.style.display="none";}
async function loadStatsFor(sym){
  var c=getContractFor(sym);if(!c)return;var def=getCurrDef(sym);
  try{
    log("Loading stats for "+sym+"...");
    var frozen=await c.sharesFrozen(),closed=await c.depositsClosed(),count=await c.payeeCount();
    var assetAddr=def.native?ZERO:def.tokenAddr,totalRcv=await c.totalReceived(assetAddr);
    var h="<div class='dist-grid three' style='margin-bottom:10px;'>";
    h+="<div class='dist-stat'><label>Payee Count</label><span>"+count.toString()+"</span></div>";
    h+="<div class='dist-stat'><label>Shares Frozen</label><span>"+(frozen?"<span class='dist-tag green'>YES</span>":"<span class='dist-tag red'>NO</span>")+"</span></div>";
    h+="<div class='dist-stat'><label>Deposits Closed</label><span>"+(closed?"<span class='dist-tag red'>YES</span>":"<span class='dist-tag green'>NO</span>")+"</span></div></div>";
    h+="<div class='dist-stat' style='margin-bottom:10px;'><label>Total Received</label><span>"+ethers.formatUnits(totalRcv,def.decimals)+" "+sym+"</span></div><div>";
    var payeeAddrs=await c.payees();
    for(var i=0;i<payeeAddrs.length;i++){
      var bps=await c.shareBps(payeeAddrs[i]);
      h+="<div class='payee-item'><code>"+payeeAddrs[i]+"</code><span>"+bps.toString()+" BPS ("+Number(bps)/100+"%)</span></div>";
    }
    if(!payeeAddrs.length)h+="<p style='color:var(--muted);font-size:12px;'>No payees set.</p>";
    h+="</div>";
    document.getElementById("statsContent-"+sym).innerHTML=h;
    log("Stats loaded for "+sym+".");
  }catch(e){log("loadStats error: "+(e.message||String(e)));}
}
async function deployAndSetPayees(sym){
  if(typeof ethers==="undefined"){alert("ethers.js not loaded. Wait and try again.");return;}
  var mm=getMetaMaskProvider();if(!mm){alert("MetaMask not detected.");return;}
  var wi=getWalletInputs(sym);if(!wi)return;
  var statusEl=document.getElementById("deployStatus-"+sym),btn=document.getElementById("deployBtn-"+sym);
  if(btn)btn.disabled=true;if(statusEl)statusEl.textContent="Connecting...";
  try{
    var _p=new ethers.BrowserProvider(mm),_s=await _p.getSigner(),owner=await _s.getAddress();
    var network=await _p.getNetwork(),chainId=Number(network.chainId);
    var explorerMap={1:"https://etherscan.io",56:"https://bscscan.com",97:"https://testnet.bscscan.com"};
    var bal=await _p.getBalance(owner);
    log("Deploying "+sym+" | bal:"+ethers.formatEther(bal)+" ETH | chain:"+chainId);
    if(statusEl)statusEl.textContent="Deploying contract...";
    var factory=new ethers.ContractFactory(DEPLOY_ABI,DEPLOY_BYTECODE,_s);
    var gasLimit;
    try{var dt=await factory.getDeployTransaction(owner);gasLimit=(await _p.estimateGas(dt))*130n/100n;}catch(ge){gasLimit=4000000n;}
    var deployed=await factory.deploy(owner,{gasLimit:gasLimit});
    log("Deploy tx: "+deployed.deploymentTransaction().hash);
    if(statusEl)statusEl.textContent="Waiting confirmation...";
    await deployed.waitForDeployment();
    var contractAddr=await deployed.getAddress();
    log("Deployed: "+contractAddr);
    var reg=getRegistry();reg[sym]=reg[sym]||{};reg[sym].contractAddr=contractAddr;reg[sym].chainId=chainId;reg[sym].deployedAt=Date.now();saveRegistry(reg);
    if(statusEl)statusEl.textContent="Setting payees...";
    var contract=new ethers.Contract(contractAddr,ABI,_s);
    var tx=await contract.setPayees(wi.addrs,wi.shares);
    log("setPayees tx: "+tx.hash);await tx.wait();log("setPayees confirmed!");
    if(statusEl)statusEl.textContent="";
    showToast(sym+" deployed & payees set!","ok");renderRegistry();
    var sa=document.getElementById("statsArea-"+sym);if(sa){sa.style.display="block";loadStatsFor(sym);}
  }catch(e){
    var msg=e.reason||e.shortMessage||e.message||String(e);
    log("deploy error: "+msg);if(statusEl)statusEl.textContent="Error: "+msg;
    showToast("Error: "+msg.slice(0,60),"error");if(btn)btn.disabled=false;
  }
}
async function doSetPayees(sym){
  var wi=getWalletInputs(sym);if(!wi)return;var c=getContractFor(sym);if(!c)return;
  try{
    log("setPayees for "+sym+"...");var tx=await c.setPayees(wi.addrs,wi.shares);
    log("Tx: "+tx.hash);await tx.wait();log("setPayees confirmed!");
    showToast("Payees saved!","ok");loadStatsFor(sym);
    var sa=document.getElementById("statsArea-"+sym);if(sa)sa.style.display="block";
  }catch(e){log("doSetPayees error: "+(e.reason||e.message));}
}
function openDC(sym){
  currentDCSym=sym;var el=document.getElementById("dcTitle");if(el)el.textContent="Deposit / Claim — "+sym;
  var panel=document.getElementById("dcPanel");if(panel){panel.style.display="block";panel.scrollIntoView({behavior:"smooth"});}
  document.getElementById("claimableDisplay").style.display="none";
}
function closeDCPanel(){var p=document.getElementById("dcPanel");if(p)p.style.display="none";currentDCSym=null;}
async function doDeposit(){
  if(!signer){alert("Connect wallet first");return;}if(!currentDCSym){alert("Select currency");return;}
  var sym=currentDCSym,def=getCurrDef(sym),c=getContractFor(sym);if(!c)return;
  var amt=document.getElementById("depositAmount").value.trim();if(!amt){alert("Enter amount");return;}
  try{
    if(def.native){var value=ethers.parseEther(amt);log("Depositing "+amt+" ETH...");var tx=await c.depositNative({value:value});log("Tx:"+tx.hash);await tx.wait();log("ETH deposit confirmed!");}
    else{
      var reg=getRegistry(),ca=(reg[sym]&&reg[sym].contractAddr)||"";
      var token=new ethers.Contract(def.tokenAddr,ERC20_ABI,signer),amount=ethers.parseUnits(amt,def.decimals);
      log("Approving "+amt+" "+sym+"...");var appTx=await token.approve(ca,amount);await appTx.wait();
      log("Approved. Depositing...");var tx2=await c.depositToken(def.tokenAddr,amount);log("Tx:"+tx2.hash);await tx2.wait();log(sym+" deposit confirmed!");
    }
    loadStatsFor(sym);showToast("Deposit confirmed!","ok");
  }catch(e){log("doDeposit error: "+(e.reason||e.message));}
}
async function checkClaimable(){
  if(!currentDCSym){alert("Select currency");return;}
  var sym=currentDCSym,def=getCurrDef(sym),c=getContractFor(sym);if(!c)return;
  var investor=document.getElementById("claimInvestor").value.trim();if(!investor){alert("Enter investor address");return;}
  try{
    var assetAddr=def.native?ZERO:def.tokenAddr,amt=await c.claimable(assetAddr,investor);
    var display=ethers.formatUnits(amt,def.decimals)+" "+sym;
    var el=document.getElementById("claimableDisplay");el.textContent="Claimable: "+display;el.style.display="block";
...
async function doClaim(){
  ...
  var sym=currentDCSym,def=getCurrDef(sym),c=getContractFor(sym);if(!c)return;
  try{
    log("Claiming "+sym+"...");var tx=def.native?await c.claimNative():await c.claimToken(def.tokenAddr);
    log("Tx:"+tx.hash);await tx.wait();log(sym+" claimed!");showToast("Claimed "+sym+"!","ok");
  }catch(e){log("doClaim error: "+(e.reason||e.message));}
}
async function doFreezeShares(){
  if(!currentDCSym||!signer){alert("Connect wallet and select currency");return;}
  if(!confirm("Freeze shares permanently?"))return;
  var c=getContractFor(currentDCSym);if(!c)return;
  try{var tx=await c.freezeShares();await tx.wait();log("Shares frozen!");loadStatsFor(currentDCSym);}
...
async function doCloseDeposits(){
  ...
  var c=getContractFor(currentDCSym);if(!c)return;
  try{var tx=await c.closeDeposits();await tx.wait();log("Deposits closed!");loadStatsFor(currentDCSym);}
  catch(e){log("closeDeposits error: "+(e.reason||e.message));}
}
function getTxLinks(){try{return JSON.parse(localStorage.getItem("tx_links")||"{}");}catch(e){return {};}}
function saveTxLinks(obj){localStorage.setItem("tx_links",JSON.stringify(obj));}
async function loadPayments(){
  log("Fetching payments...");
  try{
    var data=await api("GET","/api/v1/admin/payloads?limit=50");
    var items=data.items||data.payloads||data||[],links=getTxLinks();
    var h="<table class='pay-table'><thead><tr><th>ID</th><th>Amount</th><th>Currency</th><th>Status</th><th>TX</th><th>Action</th></tr></thead><tbody>";
    for(var i=0;i<items.length;i++){
      var p=items[i],linked=links[p.id];
      var lc=linked?"<a href='https://etherscan.io/tx/"+linked.txHash+"' target='_blank' style='color:#34d399;font-size:11px;'>View TX</a> <span class='dist-tag green'>Linked</span>":"<span style='color:var(--muted);'>&#x2014;</span>";
      h+="<tr><td style='font-size:11px;'>"+p.id+"</td><td>"+(p.amount||"&#x2014;")+"</td><td>"+(p.currency||"&#x2014;")+"</td>";
      h+="<td><span class='dist-tag "+(p.status==="completed"?"green":"yellow")+"'>"+(p.status||"&#x2014;")+"</span></td><td>"+lc+"</td>";
      h+="<td><button class='dist-btn ghost sm' data-pid='"+p.id+"' onclick='linkPayment(this.dataset.pid)'>Link</button></td></tr>";
      h+="<tr id='link-form-"+p.id+"' style='display:none;'><td colspan='6'><div style='display:flex;gap:8px;padding:6px 0;align-items:center;'>";
      h+="<input class='dist-input' id='txhash-"+p.id+"' placeholder='0x... tx hash' style='margin:0;flex:1;' />";
      h+="<button class='dist-btn success sm' data-pid='"+p.id+"' onclick='saveTxLink(this.dataset.pid)'>Save</button>";
      h+="<button class='dist-btn ghost sm' data-pid='"+p.id+"' onclick='closeLink(this.dataset.pid)'>Cancel</button>";
      h+="</div></td></tr>";
    }
    h+="</tbody></table>";
    document.getElementById("paymentsTable").innerHTML=items.length?h:"<p style='color:var(--muted);font-size:12px;'>No payments found.</p>";
    log("Loaded "+items.length+" payments.");
  }catch(e){log("loadPayments error: "+(e.message||String(e)));}
}
function linkPayment(id){var row=document.getElementById("link-form-"+id);if(row)row.style.display=row.style.display==="none"?"table-row":"none";}
function closeLink(id){var r=document.getElementById("link-form-"+id);if(r)r.style.display="none";}
function saveTxLink(id){
  var inp=document.getElementById("txhash-"+id),txHash=inp?inp.value.trim():"";
  if(!txHash||!txHash.startsWith("0x")){alert("Enter a valid tx hash starting with 0x");return;}
  var links=getTxLinks();links[id]={txHash:txHash,ts:Date.now()};saveTxLinks(links);
  log("Linked "+id+" -> "+txHash);loadPayments();showToast("Link saved!","ok");
}
function getMetaMaskProvider(){
  if(window.ethereum&&window.ethereum.providers&&window.ethereum.providers.length){
    var mm=window.ethereum.providers.find(function(p){return p.isMetaMask&&!p.isTrustWallet;});
    return mm||window.ethereum.providers[0];
  }
  return window.ethereum||null;
}
async function connectWallet(){
  if(typeof ethers==="undefined"){alert("ethers.js still loading — wait a moment and retry.");return;}
  var mm=getMetaMaskProvider();if(!mm){alert("MetaMask not detected. Install and refresh.");return;}
  var btn=document.getElementById("connectBtn"),addrEl=document.getElementById("walletAddr");
  ...
  try{
    ...
    btn.textContent="Connected";btn.style.background="#059669";btn.disabled=false;
    var bal=await provider.getBalance(walletAddress);
    var balEl=document.getElementById("walletBalance");
    balEl.textContent=parseFloat(ethers.formatEther(bal)).toFixed(4)+" ETH";balEl.style.display="inline";
    var net=await provider.getNetwork(),chainId=Number(net.chainId);
    var sel=document.getElementById("networkSelect");
    if(sel){for(var i=0;i<sel.options.length;i++){if(parseInt(sel.options[i].value)===chainId){sel.value=sel.options[i].value;break;}}}
    log("Connected: "+walletAddress+" | chain="+chainId+" | "+ethers.formatEther(bal)+" ETH");
    showToast("Wallet connected!","ok");
  }catch(e){
    btn.textContent="Connect MetaMask";btn.disabled=false;
    var msg=e.message||String(e);log("Connect error: "+msg);
    if(e.code===4001||msg.includes("rejected"))alert("Connection rejected in MetaMask.");
    else alert("MetaMask error: "+msg);
  }
}
async function switchNetwork(){
  var mm=getMetaMaskProvider();if(!mm){alert("MetaMask not found");return;}
  var chainId=parseInt(document.getElementById("networkSelect").value),hexId="0x"+chainId.toString(16);
  try{await mm.request({method:"wallet_switchEthereumChain",params:[{chainId:hexId}]});log("Switched to chain "+chainId);}
  catch(e){
    if(e.code===4902&&chainId===97){await mm.request({method:"wallet_addEthereumChain",params:[{chainId:"0x61",chainName:"BSC Testnet",nativeCurrency:{name:"BNB",symbol:"BNB",decimals:18},rpcUrls:["https://data-seed-prebsc-1-s1.binance.org:8545/"],blockExplorerUrls:["https://testnet.bscscan.com"]}]});}
    else if(e.code===4902&&chainId===56){await mm.request({method:"wallet_addEthereumChain",params:[{chainId:"0x38",chainName:"BNB Smart Chain",nativeCurrency:{name:"BNB",symbol:"BNB",decimals:18},rpcUrls:["https://bsc-dataseed.binance.org/"],blockExplorerUrls:["https://bscscan.com"]}]});}
    else{log("switchNetwork error: "+(e.message||String(e)));}
  }
}
function diagWallet(){
  var box=document.getElementById("diagBox");box.style.display="block";var lines=[];
  lines.push("<b>Wallet Diagnostics</b>");
  lines.push("ethers.js: "+(typeof ethers!=="undefined"?"Loaded v"+(ethers.version||"?"):"<span style='color:#f87171'>NOT loaded</span>"));
  lines.push("window.ethereum: "+(window.ethereum?"Detected":"<span style='color:#f87171'>NOT found</span>"));
  if(window.ethereum){
    lines.push("isMetaMask: "+!!window.ethereum.isMetaMask);
    window.ethereum.request({method:"eth_accounts"}).then(function(accs){
      lines.push(accs&&accs.length?"Connected: "+accs[0]:"Not connected");box.innerHTML=lines.join("<br>");
    }).catch(function(e){lines.push("Error: "+e.message);box.innerHTML=lines.join("<br>");});
  }
  box.innerHTML=lines.join("<br>");
}
function log(msg){
  var el=document.getElementById("distLog");if(!el)return;
  var ts=new Date().toLocaleTimeString();
  el.textContent="["+ts+"] "+msg+"\\n"+el.textContent;
  if(el.textContent.length>4000)el.textContent=el.textContent.slice(0,4000);
}
document.addEventListener("DOMContentLoaded",function(){renderRegistry();});
</script>
"""
