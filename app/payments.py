...
async def create_ledger_payment_order(payload: LedgerOrderCreate, db: AsyncSession = Depends(get_db)):
...
async def ledger_manual_confirm(payload: LedgerManualConfirm, _: AdminKey, db: AsyncSession = Depends(get_db)):
