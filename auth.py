...
async def get_me(db: AsyncSession = Depends(get_db), token: str = Depends(lambda c: c.credentials)):
    """Get current user profile."""
    from app.app.deps import get_current_user
    from fastapi.security import HTTPAuthorizationCredentials
    # Simplified — use deps in actual route
    raise HTTPException(status_code=501, detail="Use deps.get_current_user")
