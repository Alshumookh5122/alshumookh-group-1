...
class MatchingService:
    ...
    async def process_incoming_transfer(
        ...
        if payment.expires_at and payment.expires_at < datetime.datetime.utcnow():
            ...
            await db.execute(
                update(Payment).where(Payment.id == payment.id).values(status=PaymentStatus.EXPIRED)
            )
            return None
        ...
        if not cls._amount_matches(amount, expected):
            logger.warning(
                ...
                expected=str(expected), received=str(amount), payment_id=payment.id,
            )
        ...
        db.add(AuditLog(
            ...
            details={
                "tx_hash": tx_hash, "amount": str(amount),
                "token": token_symbol, "from": from_address,
            },
        ))
        ...
        try:
            ...
            confirm_crypto_transaction.apply_async(args=[payment.id, tx_hash], countdown=30)
