...
class PaymentStatus(str, enum.Enum):
    ...
    CONFIRMING = "confirming"       # Crypto: waiting for block confirmations
    COMPLETED = "completed"
    FAILED = "failed"
    CANCELLED = "cancelled"
    REFUNDED = "refunded"
...
class PaymentProvider(str, enum.Enum):
    STRIPE = "stripe"
    ALCHEMY = "alchemy"             # Crypto via Alchemy
...
class Payment(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=gen_uuid)
    reference: Mapped[str] = mapped_column(String(50), unique=True, index=True)  # Human-readable ref
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"), index=True)

    # Amount
    amount: Mapped[Decimal] = mapped_column(Numeric(20, 8), nullable=False)
    currency: Mapped[str] = mapped_column(String(10), nullable=False)  # USD, EUR, ETH, USDT…
    amount_usd: Mapped[Optional[Decimal]] = mapped_column(Numeric(20, 8))  # Normalised to USD
    ...
    provider_payment_id: Mapped[Optional[str]] = mapped_column(String(255), index=True)  # Stripe PI id
    provider_client_secret: Mapped[Optional[str]] = mapped_column(String(500))           # Stripe client secret
...
class DepositWallet(Base):
    """HD-derived deposit wallets, one per payment."""
    ...
    encrypted_private_key: Mapped[str] = mapped_column(Text)  # AES-256 encrypted
...
class BlockchainTransaction(Base):
    """On-chain transaction records linked to payments."""
    ...
    raw_data: Mapped[Optional[dict]] = mapped_column(JSON)  # Full Alchemy webhook payload
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class WebhookEvent(Base):
    """Raw webhook payloads for idempotency and debugging."""
    __tablename__ = "webhook_events"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=gen_uuid)
    provider: Mapped[str] = mapped_column(String(30), index=True)
    event_type: Mapped[str] = mapped_column(String(100), index=True)
    event_id: Mapped[Optional[str]] = mapped_column(String(255), index=True)  # Provider event ID
