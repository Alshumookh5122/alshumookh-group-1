...
All webhook payloads are persisted first (idempotent), then processed.
"""
...
async def _save_webhook_event(db, provider: str, event_type: str, event_id: str, payload: dict) -> WebhookEvent:
    """Persist raw webhook payload before processing (idempotency check)."""
...
async def alchemy_webhook(
    ...
    """
    Handles Alchemy ADDRESS_ACTIVITY webhook events.
    Validates HMAC signature, then routes to the matching engine.
    """
...
async def _process_alchemy_address_activity(db: AsyncSession, payload: Dict[str, Any]):
    """
    Parse ADDRESS_ACTIVITY event and pass each transfer to the matching service.
    Alchemy event structure:
      payload.event.activity[] — list of transfer activities
    """
    ...
    for activity in activities:
        ...
        # Only process incoming transfers (to our addresses)
        ...
        token_contract = raw_contract.get("address") if raw_contract else None
...
async def stripe_webhook(
    ...
    Validates signature using the Stripe SDK, then updates payment records.
    """
    import stripe
...
async def _process_stripe_event(db: AsyncSession, event: Dict[str, Any]):
    """Update payment status based on Stripe event type."""
    import datetime
